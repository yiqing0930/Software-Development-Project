...
if(isset($_SESSION['email'])){
    // Construct the SQL query to search for products with titles matching the search query
    ...
    AND Item.title LIKE '%$search_query%'";
}else{
    // Construct the SQL query to search for products with titles matching the search query
    $sql = "SELECT Item.*, Category.categoryType FROM Item 
    INNER JOIN Category ON Item.categoryID = Category.categoryID 
    WHERE Item.title LIKE '%$search_query%'";
}
...
if ($result->num_rows > 0) {
    // Output each search result with the same product layout
    while ($row = $result->fetch_assoc()) {
        ...
        <head>
            ...
            <title>UWE SwapMe - Search Results</title>
            <style>
                .banner_section {
...
// End output buffering and store the HTML content in a variable
