...
$sql_latest_id = "SELECT itemID FROM Item ORDER BY itemID DESC LIMIT 1";
$result_latest_id = $conn->query($sql_latest_id);
if ($result_latest_id->num_rows > 0) {
    $latest_id = $result_latest_id->fetch_assoc()['itemID'];
    // Extract the numeric part of the latest item ID and increment it
    $latest_numeric_part = intval(substr($latest_id, 2));
    $next_numeric_part = $latest_numeric_part + 1;
    // Generate the next item ID with leading zeros
    $item_ID = 'IT' . sprintf('%02d', $next_numeric_part);
} else {
    // If no previous item exists, start from IT01
    $item_ID = 'IT01';
}
...
if(isset($_FILES['image'])) {
    ...
    $fileContent = './upload_img/' . $fileName;
}

// Get publisher's email from session
$publisherEmail =  $_SESSION['email'];

// Prepare an SQL INSERT statement to insert the form data into the 'item' table
$sql = "INSERT INTO Item (itemID, title, description, imageURL, price, categoryID, publisherEmail) VALUES (?, ?, ?, ?, ?, ?, ?)";
$stmt = $conn->prepare($sql);

// Bind parameters
$stmt->bind_param("ssssdss", $item_ID, $title, $description, $fileContent, $price, $category, $publisherEmail);

// Execute the statement
if ($stmt->execute()) {
    // Display JavaScript to show the modal
    echo "<script>$(function(){showUploadModal();});</script>";
    ...
    <div class="container mt-5">
            ...
            <div class="big-form-container">
                ...
                <div class="form-container">
                    ...
                    <input type="number" name="price" id="price" min="1" value="10.00" step=".01">
                </div>
