...
<body>
    ...
    <div class="container-fluid mt-5">
        <div class="row">
            <!-- Narrow container on the left (col-sm-3) -->
            <div class="col-sm-3">
                <!-- Search form and category filter form here -->
            ...
            <div class="col-sm-9">
                <div class="row">
                    ...
                    if(isset($_GET['category'])){
                        ...
                        // Construct the SQL query to filter items based on selected categories
                        $sql = "SELECT Item.*, Category.categoryType FROM Item 
                        INNER JOIN Category ON Item.categoryID = Category.categoryID";

                        if(isset($_SESSION['email'])){
                            // If categories are selected, add a WHERE clause to filter by categories
                            if (!empty($selected_categories)) {
                            $sql .= " WHERE Category.categoryType IN ('" . implode("', '", $selected_categories) . "') AND Item.publisherEmail != '{$_SESSION['email']}'";
                            }
                        }else{
                            // If categories are selected, add a WHERE clause to filter by categories
                            if (!empty($selected_categories)) {
                                $sql .= " WHERE Category.categoryType IN ('" . implode("', '", $selected_categories) . "')";
                                }
                        }
                        ...
                        switch ($sortBy) {
                            ...
                            case 'z-a':
                                ...
                                break;
                            default:
                                // Default sorting if criteria is invalid
